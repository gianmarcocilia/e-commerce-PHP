...
if (isset($_POST['create'])) {
    if (isset($_POST['category_id'])) {
        ...
        if ($validated) {
            $products_class->setCreatedValue($values);
            $products_class->create();
...
<h2 class="py-5">Crea un nuovo Prodotto</h2>
<form method="POST" class="row g-3">
    <div class="col-md-6">
        <label for="name" class="form-label">Nome</label>
        <input type="text" class="form-control <?php isset($errors) ? isValid($errors['name']) : '' ?>" id="name" value="<?php echo isset($_POST['name']) ? $_POST['name'] : ''?>" name="name">
        <?php isset($errors) ? errorMessage($errors['name']) : '' ?>
    </div>
    <div class="col-md-6">
        <label for="price" class="form-label">Prezzo €</label>
        <input type="number" class="form-control <?php isset($errors) ? isValid($errors['price']) : '' ?>" value="<?php echo isset($_POST['price']) ? $_POST['price'] : ''?>" id="price" name="price">
    ...
    <div class="col-md-6">
        ...
        <input name="image" type="text" class="form-control <?php isset($errors) ? isValid($errors['image']) : '' ?>" value="<?php echo isset($_POST['image']) ? $_POST['image'] : ''?>" id="image" aria-describedby="image">
    ...
    <div class="col-md-6">
        ...
        <select name="category_id" class="form-select" id="category" aria-describedby="category">
            ...
            <?php foreach ($categories as $category) : ?>
                <option <?php if(isset($_POST['category_id']) && $_POST['category_id'] == $category->id){echo 'selected';} ?> value="<?php echo $category->id; ?>"><?php echo $category->name ?></option>
            <?php endforeach; ?>
        </select>
